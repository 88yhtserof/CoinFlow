...
private extension CoinIndicatorCollectionViewCell {
    ...
    func configureView() {
        ...
        marketCapInfoView.valueLabel.text = CoinNumberFormatter.currentPrice(number: 142060908).string
        
        fullyDilutedValuationInfoView.titleLabel.text = "완전 희석 가치(FDV)"
        fullyDilutedValuationInfoView.valueLabel.text = CoinNumberFormatter.currentPrice(number: 142060908).string
        
        totalVolumeInfoView.titleLabel.text = "총 거래량"
        totalVolumeInfoView.valueLabel.text = CoinNumberFormatter.currentPrice(number: 142060908).string
